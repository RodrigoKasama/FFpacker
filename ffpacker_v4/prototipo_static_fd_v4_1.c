...
typedef struct
{
	...
	double framerate;
	int width;
	int height;xxx.yyy.zzz
	int vStreamidx;
} tmpinfo_t;

// gcc -o main static_fd_v4_1.c -lavformat -lavutil -lavcodec
...
int main(int argc, char **argv)
{
	...
	*/
	...
	// }
	int complete_tmp, temp_frames = count_tmpframes(TEMP_FILE);
	if (temp_frames > 0)
	{
		fprintf(stdout, "No arquivo temporario foram encontrados %d frames, gerando arquivo...\n", temp_frames);
		aux_file.nb_frames = temp_frames;
		complete_tmp = 1;
	}
	else
	{
		fprintf(stdout, "Nao foram encontrados frames suficientes no arquivo temporario..\n");
		aux_file.nb_frames = 0;
		complete_tmp = 0;
	}
