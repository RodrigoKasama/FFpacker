...
// gcc -o main ffpacker_v5.c -lavformat -lavutil -lavcodec
// ./main 'rtsp://xxx.yyy.zzz' tcp "$PWD" 10


/**
 * @brief Move o arquivo temporario para um novo arquivo com nome baseado nas propriedades do video.
 * 
 * @param tmp_file Estrutura para controle do arquivo temporario (buffer, tamanho do buffer, nº de frames escritos, framerate, dimensões do video, indice da stream de video, status do arquivo - completo ou incompleto).
 * @return int 
 */
int move_tmpf(tmpinfo_t *tmp_file){
	...
	sprintf(filename, "File_%05d_%s_%d_%dx%d_%.2f.h264", ++(tmp_file->id), tmp_file->status_text, tmp_file->nb_frames, tmp_file->width, tmp_file->height, tmp_file->framerate);
...
/**
 * @brief Conta o número de frames em um arquivo de video.
 * 
 * @param file 
 * @return int 
 */
int count_tmpframes(char* file){
...
/**
 * @brief Coleta o framerate de um arquivo de video.
 * 
 * @param file_input 
 * @param video_index 
 * @return double 
 */
double get_framerate(char* file_input, int video_index){
	...
	if (avformat_open_input(&fmt_ctx, file_input, NULL, NULL) != 0){
...
/**
 * @brief Escreve o buffer em um arquivo temporario, reutilizando o file descriptor em uso. 
 * 
 * @param temp_arq Estrutura para controle do arquivo temporario (buffer, tamanho do buffer, nº de frames escritos, framerate, dimensões do video, indice da stream de video, status do arquivo - completo ou incompleto).
 * @param move_flag Caso move_flag seja verdadeiro, coleta o framerate do arquivo temporario, fecha o arquivo, renomeia o arquivo com as propriedades da stream (Nº de frames, Framerate, Completo ou interrompido, dimensões do video, etc) e cria um novo arquivo temporario para escrita, com o mesmo file descriptor.
 * @return int 
 */
int write_temp_file(tmpinfo_t *temp_arq, int move_flag){

	// Escreve o keyframe apenas no inicio de cada arquivo..
	// Verifica no segundo argumento se escreve o pacote ou se fecha e escreve em um novo
	...
	if (move_flag){
		...
		// Pegar o framerate, fecha e move...

		// Não inverter a ordem desses dois comandos, do contrário o file descriptor não será reutilizado
		...
		// Abre um novo aquivo temporario
		fd = open(TEMP_FILE, O_CREAT | O_WRONLY | O_TRUNC | O_SYNC, S_IRWXU | S_IRWXG | S_IRWXO);
		if (fd < 0){
			fprintf(stderr, "Erro em instanciar o arquivo temp.\n");
		}
...
/**
 * @brief Trata arquivos temporarios incompletos. Coleta as informações da stream do arquivo temporário (Nº de frames, framerate) e realiza o processo de fechamento da stream.
 * 
 * @param tmp_interr Estrutura para controle do arquivo temporario (buffer, tamanho do buffer, nº de frames escritos, framerate, dimensões do video, indice da stream de video, status do arquivo - completo ou incompleto).
 * @param interr_text String indicando o status daquele vídeo (Completo ou incompleto).
 * @return int
 */
int close_interr_tmp(tmpinfo_t* tmp_interr, char* interr_text){

	int n_frames = count_tmpframes(TEMP_FILE);
	if (n_frames > 0)
	{
		tmp_interr->nb_frames = n_frames;
		tmp_interr->status_text = interr_text;
		tmp_interr->framerate = get_framerate(TEMP_FILE, tmp_interr->vStreamidx);
		move_tmpf(tmp_interr);
	}
	return n_frames;
}
...
int main(int argc, char **argv){
	...
	int vstream_index = -1, temp_frames = -1;
	...
	if (!vStream){
		exit_error("Couldn't find video stream from input, aborting\n");
	}
	...
	*/
	...
	// Verifica se ha arquivos temporarios incompletos
	fprintf(stdout, "Buscando frames remanescentes.. ");
	
	// Preenche os dados pendentes (framerate, nb_frames, define o texto de apresentacao e no cod_stat do arquivo) e move o arquivo
	temp_frames = close_interr_tmp(&aux_file, "incompleto");

	if (temp_frames == -1) {
		// exit_error("Erro ao ler o arquivo temporario..\n");
		aux_file.nb_frames = 0;
	} else if (temp_frames == 0) {
		fprintf(stdout, " sem frames remanescentes.\n");
	} else {
		fprintf(stdout, " arquivo com %d frames gerado.\n", temp_frames);
	}

	// A partir desse ponto não há arquivos temporarios incompletos 
	aux_file.status_text = "completo";
	aux_file.nb_frames = 0;

	int one_frame_file = 0, move_file = 0;
	while (av_read_frame(in_fmt_ctx, pPkt) >= 0)
	{

		if (pPkt->stream_index == vstream_index){

			// Coleta o Pacote(FRAME UNICO) e seu tamanho
			aux_file.buffer = (char *)pPkt->buf->data;
			aux_file.bufsize = pPkt->buf->size;

			// Define se o arquivo sera movido ou nao
			//		!(Se ha mais de um pacote escrito e o pacote atual e um KEYFRAME), define para mover o arquivo
			//		Senao, define para escrever no arquivo
			if (!(pPkt->flags == AV_PKT_FLAG_KEY && one_frame_file != 0)){
				move_file = 0;
				one_frame_file = 1;
			} else {
				move_file = 1;
