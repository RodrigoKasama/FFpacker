...
// gcc -o main static_fd_v2.c -lavformat -lavutil -lavcodec
...
int write_temp_fil(void *dados, tmpinfo_t *temp_arq, int close_flag){
...
int write_temp_file(void *dados, tmpinfo_t *temp_arq, int close_flag)
{
	...
	*/
	...
	// Fechar o atual (pega o framerate, fecha e move) + Abre um novo e escreve.
	...
	if (close_flag)
	{
		...
		// Abre um novo aquivo temp, o anterior foi movido com a descricao
		fd = open(TEMP_FILE, O_CREAT | O_WRONLY | O_TRUNC | O_SYNC, S_IRWXU | S_IRWXG | S_IRWXO);
		if (fd < 0)
		{
			/* USAR perror() */
...
int main(int argc, char **argv)
{
	...
	if (chdir(argv[3]) != 0)
	{
		fprintf(stderr, "Nao foi possivel ir para o diretorio '%s'.\n", argv[3]);
		exit(1);
	}
	...
	*/
	...
	while (av_read_frame(in_fmt_ctx, pPkt) >= 0)
	{
		...
		if (pPkt->stream_index == vstream_index)
		{
			...
			write_temp_file(pPkt, &aux_file, move_file);
